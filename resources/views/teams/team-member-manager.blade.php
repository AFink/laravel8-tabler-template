    ...
    @if (Gate::check('addTeamMember', $team))
        ...
        <x-jet-form-section submit="addTeamMember">
            ...
            <x-slot name="form">
                ...
                <div class="mb-3">
                    {{ __('Please provide the email address of the person you would like to add to this team. If the invited account doesn\'t exist yet, the person will still be invited.') }}
                </div>
                    ...
                    <div class="my-3 w-md-75">
                        ...
                        <div class="list-group">
                            @foreach ($this->roles as $index => $role)
                                <a href="#"
                                    class="list-group-item list-group-item-action{{ isset($addTeamMemberForm['role']) && $addTeamMemberForm['role'] !== $role->key ? ' text-black-50' : '' }}"
                                    wire:click.prevent="$set('addTeamMemberForm.role', '{{ $role->key }}')">
                                    <div>
                                        <span
                                            class="{{ $addTeamMemberForm['role'] == $role->key ? 'font-weight-bold' : '' }}">
                                            {{ $role->name }}
                                        </span>
                                        @if ($addTeamMemberForm['role'] == $role->key)
                                            <svg class="ml-1 text-success font-weight-light" width="20" fill="none"
    ...
    <x-jet-dialog-modal wire:model="currentlyManagingRole">
        ...
        <x-slot name="content">
            <div class="list-group">
                @foreach ($this->roles as $index => $role)
                    <a href="#"
                        class="list-group-item list-group-item-action{{ $currentRole !== $role->key ? ' text-black-50' : '' }}"
